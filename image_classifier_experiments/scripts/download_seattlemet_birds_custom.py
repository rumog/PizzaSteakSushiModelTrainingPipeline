...
DATA_DIR = "data"
SOURCE_IMG_DIR = "seattlemet_birds/images"
SOURCE_META_DIR = "seattlemet_birds/meta"
# Setup data paths
...
def get_subset(
    ...
    target_classes=["osprey", "mallard", "stellers_jay"],
    amount=0.1,
    seed=42,
):
    random.seed(seed)
...
def download_seattlemet_birds_custom(
    data_dir: str,
    source_img_dir: str,
    source_meta_dir: str,
    target_classes: list[str] = ["osprey", "mallard", "stellers_jay"],
    amount=0.01,
):
    ...
    if len(target_classes) <= 3:
        ...
        sub_dir = f"seattlement_birds_{len(target_classes)}"
...
""
data_path = Path(DATA_DIR)
classes_file = data_path / SOURCE_META_DIR / "classes.txt"
class_list = []
...
class_list = [
    ...
    "darkeyed_junco",
    "northern_shoveler",
    "goldencrowned_sparrow",
    "greenwinged_teal",
    "common_merganser",
    "redwinged_blackbird",
]
# download_seattlemet_birds_custom(DATA_DIR, SOURCE_IMG_DIR, SOURCE_META_DIR, class_list, 1)

# walk_through_dir("data/seattlement_birds_10_100_percent")

print(f"num workers = {os.cpu_count()}")
